$(window).load(function() {
  ...
  function getBlogArticleMarkup(post) {
    ...
    for(var j = 0; j < post.photos.length; j++) {
      ...
      html += '<li>';
      html += '<a href="' + remapUrl(post.photos[j].url) + '" class="colorbox">';
      html += '  <img src="' + remapUrl(image_url) + '" class="img-responsive clearfix" />';
      html += '</a>';
      html += '<a href="//pinterest.com/pin/create/button/?url=' + encodeURIComponent(post.post_url);
      html += '&media=' + encodeURIComponent(remapUrl(image_url));
  ...
  // Some URLS have been changed in Tumblr. This is a HACK to fix that.
